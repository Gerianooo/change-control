        ...
        <DataTable v-if="a" :url="route('api.v1.revision.paginate', document.id)" :sticky="true">
          ...
          <template v-slot:tbody="{ data, refresh }">
            <tr v-for="(revision, i) in data" :key="i">
              ...
              <td class="border border-slate-200 px-2 py-1">{{ revision.code }}</td>
              <td class="border border-slate-200 px-2 py-1">{{ new Date(revision.created_at).toLocaleString('id') }}</td>
              <td class="border border-slate-200 px-2 py-1">{{ new Date(revision.updated_at).toLocaleString('id') }}</td>
              <td class="border border-slate-200 px-2 py-1">{{ new Date(revision.expired_at).toLocaleString('id') }}</td>
