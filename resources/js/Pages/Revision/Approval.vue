...
<template>
  <DashboardLayout :title="__('Revision Approval')">
        ...
        <div class="p-4">
          <div class="rounded-md overflow-auto border border-slate-300 dark:border-gray-900">
            <table class="w-full border-collapse">
              <thead class="bg-slate-200 dark:bg-gray-800 border-slate-300 dark:border-gray-900">
                <tr class="bg-inherit border-inherit">
                  <th class="border border-inherit py-2 text-center uppercase" rowspan="2">{{ __('no') }}</th>
                  <th v-for="(approver, i) in approvers" :key="i" class="border border-inherit text-center font-semibold uppercase px-2" colspan="3">
                    {{ approver.user.name }}
                  </th>
                </tr>
              ...
              <tbody>
                <tr v-for="(approve, i) in approves" :key="i" class="hover:bg-slate-100 dark:hover:bg-gray-600 border-slate-300 dark:border-gray-800 transition-all duration-300">
  ...
  <Modal :show="note !== ''">
    <Card class="bg-white dark:bg-gray-700 dark:text-gray-200 w-full max-w-xl shadow-xl">
      <template #header>
        <div class="flex items-center justify-end space-x-2 bg-slate-200 dark:bg-gray-800 p-2">
