...
const { approvers, revision, users } = defineProps({
  approvers: Array,
  revision: Object,
  users: Array,
})
...
const store = () => {
  return form.post(route('revision.approver.add', {
    revision: revision.id,
    user: form.user
  }), {
    onSuccess: () => {
      Inertia.get(route(route().current(), revision.id))
    },
    onError: () => show(),
  })
}
...
const update = () => {
  return form.patch(route('revision.approver.update', {
    approver: form.id,
    user: form.user,
  }), {
    onSuccess: () => {
...
const detach = async approver => {
  ...
  response.isConfirmed && Inertia.delete(route('revision.approver.detach', approver.id), {
    onSuccess: () => {
      Inertia.get(route(route().current(), revision.id))
    },
  })
...
const change = () => {
  ...
  }).patch(route('revision.approver.save', revision.id), {
    onSuccess: () => {
      Inertia.get(route(route().current(), revision.id))
      Swal.close()
    },
