...
<template>
  <div
    ...
    @dragover.prevent=""
    @drop="drop(procedure)"
    class="flex items-center space-x-2 rounded-md px-4 bg-slate-200 hover:bg-slate-100 dark:bg-gray-800 dark:hover:bg-gray-600 transition-all"
    :draggable="true"
    :id="procedure.id">
    ...
    <div ref="container" class="flex-none flex items-center rounded-md" :draggable="false">
      <Icon v-if="procedure.parent_id" name="arrow-left" @click.prevent="left" class="px-2 py-1 bg-slate-100 hover:bg-white border border-slate-300 text-gray-700 transition-all cursor-pointer" />
      <Icon v-if="procedure.position > 1" name="arrow-right" @click.prevent="right" class="px-2 py-1 bg-slate-100 hover:bg-white border border-slate-300 text-gray-700 transition-all cursor-pointer" />
      <Icon v-if="procedure.position > 1" name="arrow-up" @click.prevent="up" class="px-2 py-1 bg-slate-100 hover:bg-white border border-slate-300 text-gray-700 transition-all cursor-pointer" />
      <Icon v-if="procedure.parent?.childs_count !== procedure.position" name="arrow-down" @click.prevent="down" class="px-2 py-1 bg-slate-100 hover:bg-white border border-slate-300 text-gray-700 transition-all cursor-pointer" />
